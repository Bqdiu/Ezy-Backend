...
const getReturnRequest = async (req, res) => {
    const { shop_id, return_type_id, user_order_id, limit = 5, page = 1 } = req.body;
    ...
    let whereCondition = { shop_id, return_type_id };
    if (user_order_id) {
        whereCondition = { ...whereCondition, user_order_id };
    }
...
const acceptReturnRequest = async (req, res) => {
    ...
    try {
        ...
        if (orderGHNDetails.status === "delivered") {
            ...
            if (
                returnRequest.return_reason_id === 1 ||
                returnRequest.return_reason_id === 2
            ) {
            ...
            if (resultGHN.data) {
                ...
                if (
                    ...
                    final_price =
                        order.final_price -
                        (order.shipping_fee - order.discount_shipping_fee);
                } else {
        ...
        if (
            orderGHNDetails.status === "ready_to_pick" ||
            orderGHNDetails.status === "picking"
        ) {
        ...
        return res.status(400).json({
            ...
            notification:
                "Không thể chấp nhận yêu cầu trả hàng với đơn hàng đang được giao",
        });
    } catch (error) {
...
async function adjustStockAndSales(order) {
    if (order.order_code !== null) {
        ...
        if (cancelGHNResult.error) {
            return res.status(400).json({
                error: true,
                message: "Câp nhật trạng thái đơn hàng thất bại, vui lòng thử lại sau",
            });
        }
    }
